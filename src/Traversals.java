...
public class Traversals {
  ...
  public static <T> String buildPostOrderString(TreeNode<T> node) {
    ...
    String leftTree = buildPostOrderString(node.left);
    String rightTree = buildPostOrderString(node.right);

    return leftTree + rightTree + node.value;
  }
  ...
  public static int countDistinctValues(TreeNode<Integer> node) {
    ...
    uniqueVals(node, result);

    return result.size();
  }

  public static void uniqueVals(TreeNode<Integer> node, Set<Integer> result) {
    if (node == null) {
      return;
    }
    ...
    uniqueVals(node.left, result);
    uniqueVals(node.right, result);
  }
  ...
  public static boolean hasStrictlyIncreasingPath(TreeNode<Integer> node) {
    ...
    return higherThanPrev(node, Integer.MIN_VALUE);
  }

  public static boolean higherThanPrev(TreeNode<Integer> node, int previousValue) {
    if (node == null) {
      return false;
    }
    ...
    return higherThanPrev(node.left, previousValue) || higherThanPrev(node.right, previousValue);
  }
